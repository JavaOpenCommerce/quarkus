...
public class CardController {
    ...
    public void redisGetTest(RoutingContext rc) {
        ...
        if (nonNull(rc.session())) {
            productList = (List<Product>) ofNullable(rc.session().get("card")).orElse(emptyList());
        } else {
            productList = emptyList();
        }

        cardDtoService.getCard(emptyList(), rc).subscribe().with(card -> {
            rc.response()
                    .putHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON)
                    .end(Json.encodePrettily(card), AsyncResult::succeeded);
